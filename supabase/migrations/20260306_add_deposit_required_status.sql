-- Add 'deposit_required' to VIP booking status constraints
-- New flow: submitted → in_review → deposit_required → confirmed
--                                  ↘ confirmed (no deposit)
--                                  ↘ rejected
--           any non-terminal → cancelled

-- 1. Update CHECK constraint on vip_booking_requests.status
ALTER TABLE public.vip_booking_requests
  DROP CONSTRAINT IF EXISTS vip_booking_requests_status_check,
  ADD CONSTRAINT vip_booking_requests_status_check
    CHECK (status IN ('submitted','in_review','deposit_required','confirmed','rejected','cancelled'));

-- 2. Update CHECK constraint on vip_booking_status_events.to_status
ALTER TABLE public.vip_booking_status_events
  DROP CONSTRAINT IF EXISTS vip_booking_status_events_to_status_check,
  ADD CONSTRAINT vip_booking_status_events_to_status_check
    CHECK (to_status IN ('submitted','in_review','deposit_required','confirmed','rejected','cancelled'));

-- 3. Re-create admin_update_vip_booking_request RPC to accept deposit_required
...
BEGIN
  ...
  IF p_patch ? 'status' THEN
    ...
    IF v_status NOT IN ('submitted', 'in_review', 'deposit_required', 'confirmed', 'rejected', 'cancelled') THEN
  ...
  -- Settle agent tasks on terminal statuses (deposit_required is NOT terminal)

-- VIP dashboard admin edits with atomic audit logging.

CREATE TABLE IF NOT EXISTS public.vip_booking_edit_audits (
...
CREATE INDEX IF NOT EXISTS idx_vip_booking_edit_audits_booking_request_id
  ON public.vip_booking_edit_audits(booking_request_id, created_at DESC);
...
BEGIN
  ...
  IF p_patch ? 'status' THEN
    ...
    IF v_status <> v_request.status THEN
      IF v_request.status = 'submitted' AND v_status IN ('in_review', 'confirmed', 'rejected', 'cancelled') THEN
        v_allowed := true;
      ELSIF v_request.status = 'in_review' AND v_status IN ('confirmed', 'rejected', 'cancelled') THEN
        v_allowed := true;
      ELSIF v_request.status = 'confirmed' AND v_status = 'cancelled' THEN
        v_allowed := true;
      ELSE
        v_allowed := false;
      END IF;
      ...
      IF NOT v_allowed THEN
        RAISE EXCEPTION 'Invalid status transition: % -> %', v_request.status, v_status;
      END IF;
  ...
  IF v_new_status IN ('confirmed', 'rejected', 'cancelled') THEN
...
ALTER TABLE public.vip_booking_edit_audits ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS service_role_all_vip_booking_edit_audits ON public.vip_booking_edit_audits;
CREATE POLICY service_role_all_vip_booking_edit_audits
  ON public.vip_booking_edit_audits
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);

REVOKE ALL ON TABLE public.vip_booking_edit_audits FROM anon;
REVOKE ALL ON TABLE public.vip_booking_edit_audits FROM authenticated;

GRANT SELECT, INSERT, UPDATE, DELETE
  ON TABLE public.vip_booking_edit_audits
  TO service_role;

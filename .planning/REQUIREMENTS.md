...
- [x] **VPRC-07**: MCP tool queries events for the requested date and returns event context / busy night signal
- [x] **VPRC-08**: MCP tool returns `pricing_approximate` flag so agent can modulate language ("around" vs "exactly")
...
- [x] **LIFE-01**: Old `get_vip_table_availability` and `get_vip_table_chart` tools removed from server registration
...
| VPRC-07 | Phase 3 | Complete |
| VPRC-08 | Phase 3 | Complete |
...
| EMBR-01 | Phase 4 | Pending |
| EMBR-02 | Phase 4 | Pending |
| EMBR-03 | Phase 4 | Pending |
...
*Last updated: 2026-03-11 — EMBR-01/02/03 reset to Pending, reassigned to Phase 4 (gap closure)*
